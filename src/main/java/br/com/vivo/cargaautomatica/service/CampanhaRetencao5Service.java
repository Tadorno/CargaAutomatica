...
public class CampanhaRetencao5Service implements ICampanhaService{
    ...
    public void realizarCarga() {
        Logger.getLogger(CampanhaRetencao5Service.class.getName()).log(Level.INFO, "Iniciando carga de campanha.");

        File file;
        try {
            file = FileUtil.copyFile(
                    PropertySingleton.getProperty("file-crm_ret_5-origem-path"),
                    PropertySingleton.getProperty("file-crm_ret_5-origem-path"),
                    PropertySingleton.getProperty("file-crm_ret_5-origem-path")
            );
            ...
            Logger.getLogger(CampanhaRetencao5Service.class.getName()).log(Level.INFO, "Iniciando leitura do arquivo de carga para a persistência");

            Connection con = ConnectionFactory.getConnection(DataBaseEnum.GEN_OCS);
            while (sc.hasNextLine()) {
                contatosDisponibilizados++;
                
                Logger.getLogger(CampanhaRetencao5Service.class.getName()).log(Level.INFO, "Processando Linha {0}", new Object[]{contatosDisponibilizados});
                ...
                try{
                    ...
                    Logger.getLogger(CampanhaRetencao5Service.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            ...
            sc.close();
            file.delete();
            Logger.getLogger(CampanhaRetencao5Service.class.getName()).log(Level.INFO, "Carga finalizada. Total informado: {0} - Total com sucesso: {1}", new Object[]{contatosDisponibilizados, contadosCarregados});
